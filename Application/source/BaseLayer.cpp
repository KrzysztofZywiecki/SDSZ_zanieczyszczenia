...
void BaseLayer::Update(float frameTime)
{
    ...
    text->UpdateText(std::string("Sample number - " + Library::Text::FloatToString(float(sampleNumber)/1000.0,2)) + "k\nWith "
        + Library::Text::FloatToString(samplesPS, 0) + " samples per second\nTime - " + Library::Text::FloatToString(float(sampleNumber)/(diffusionSamples * windSamples), 2));
